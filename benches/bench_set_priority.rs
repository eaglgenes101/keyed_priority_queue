...
pub fn bench_set_priority(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("binary_set_priority_usize");
    ...
    let mut group = c.benchmark_group("weak_set_priority_usize");
    for &size in &[10_000, 500_000] {
        assert!(base_keys.len() >= size);
    ...
    let mut group = c.benchmark_group("binary_set_priority_string");
    ...
    let mut group = c.benchmark_group("weak_set_priority_string");
    let base_keys = get_random_strings(50_000, 0);
    let base_values = get_random_strings(50_000, 7);
